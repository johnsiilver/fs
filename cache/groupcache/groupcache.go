...
// OpenFile implements fs.OpenFiler.OpenFile(). We support os.O_CREATE, os.O_RDONLY, os.O_WRONLY,
...
func (f *FS) OpenFile(name string, flags int, options ...jsfs.OFOption) (fs.File, error) {
	...
	if !isFlagSet(flags, os.O_EXCL) {
		return nil, fmt.Errorf("groupcache does not support os.O_EXCL")
	}
...
func (f *FS) WriteFile(name string, content []byte, perm fs.FileMode) error {
	...
	file, err := f.OpenFile(name, os.O_WRONLY|os.O_CREATE|os.O_TRUNC)
	if err != nil {
		return err
	}
