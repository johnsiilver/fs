...
func newIndex(location string, olderThan time.Duration) *index {
	return &index{
		expires:   llrb.New(),
		location:  location,
		olderThan: olderThan,
...
func (i *index) expireItem(item llrb.Item) bool {
	ek := item.(expireKey)
	i.expires.Delete(ek)
	name := filepath.Join(i.location, strings.Replace(ek.name, "/", "_slash_", -1))
	if err := os.Remove(name); err != nil {
		log.Println("error removing file: ", err)
	}
	log.Println("Removing expired: ", ek.name)
	return true
}
